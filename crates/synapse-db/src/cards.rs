...
mod tests {
    ...
    fn setup() -> (SqliteStorage, i64, i64) {
        ...
        s.import(&CanonicalModel {
            notetypes: vec![Notetype { id: 1, name: "Basic".into(), kind: 0, mod_ms: 0, usn: -1, config_json: "{}".into() }],
            fields: vec![
                Field { notetype_id: 1, ord: 0, name: "Front".into(), config_json: "{}".into() },
                Field { notetype_id: 1, ord: 1, name: "Back".into(), config_json: "{}".into() },
            ],
            templates: vec![Template { notetype_id: 1, ord: 0, name: "Card 1".into(), qfmt: "{{Front}}".into(), afmt: "{{Back}}".into(), config_json: "{}".into() }],
            notes: vec![Note { id: 1, guid: "g1".into(), notetype_id: 1, mod_ms: 0, usn: -1, tags: vec![], fields: vec!["A".into(), "B".into()], sort_field: None, checksum: None }],
            cards: vec![Card { id: 1, note_id: 1, deck_id: 1, ord: 0, mod_ms: 0, usn: -1, ctype: 0, queue: 0, due: 1, interval: 0, ease_factor: 0, reps: 0, lapses: 0, remaining: 0, original_due: 0, original_deck_id: 0, flags: 0, fsrs_stability: None, fsrs_difficulty: None, fsrs_last_review: None, data: None }],
            ..Default::default()
        }).unwrap();
        let nt_id: i64 = s.lock().query_row("SELECT id FROM notetypes WHERE name = 'Basic'", [], |r| r.get(0)).unwrap();
        let note_id: i64 = s.lock().query_row("SELECT id FROM notes WHERE notetype_id = ?1", [nt_id], |r| r.get(0)).unwrap();
        let card_id: i64 = s.lock().query_row("SELECT id FROM cards WHERE note_id = ?1", [note_id], |r| r.get(0)).unwrap();
        (s, note_id, card_id)
    }
    ...
    fn suspend_and_unsuspend() {
        ...
        let q_before: i64 = s.lock().query_row("SELECT queue FROM cards WHERE id = ?1", [card_id], |r| r.get(0)).unwrap();
        assert_eq!(q_before, 0);

        s.suspend_cards(&[card_id]).unwrap();
        let q: i64 = s.lock().query_row("SELECT queue FROM cards WHERE id = ?1", [card_id], |r| r.get(0)).unwrap();
        assert_eq!(q, -1);

        // Suspended card is excluded from study queue
        let queue = s.study_queue(1, 0, 1_700_000_000_000, 20, 200).unwrap();
    ...
    fn set_flag_persists() {
        ...
        let flag: i64 = s.lock().query_row("SELECT flags FROM cards WHERE id = ?1", [card_id], |r| r.get(0)).unwrap();
        assert_eq!(flag, 3);
    }
    ...
    fn add_note_tag_idempotent() {
        ...
        let tags: String = s.lock().query_row("SELECT tags FROM notes WHERE id = ?1", [note_id], |r| r.get(0)).unwrap();
