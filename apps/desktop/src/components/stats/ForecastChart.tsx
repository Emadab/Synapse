...
export function ForecastChart({ forecast, backlogCount }: { forecast: DayCount[]; backlogCount: number }) {
  ...
  return (
    <div className="relative">
      ...
      <p className="mb-2 text-xs text-muted-foreground">
        <span className="font-medium text-foreground">{dueTotal.toLocaleString()}</span> cards due in the
        next 30 days
        {backlogCount > 0 ? (
          <>
            {" "}
            (<span className="font-medium text-foreground">{backlogCount.toLocaleString()}</span> overdue)
          </>
        ) : null}
      </p>
