...
fn ipc_err(msg: impl std::fmt::Display) -> IpcError {
    IpcError { kind: IpcErrorKind::Storage, message: msg.to_string() }
}

#[tauri::command]
pub fn suspend_cards(
...
pub fn set_card_flag(
    ...
    if flag > 7 {
        return Err(IpcError { kind: IpcErrorKind::Invalid, message: format!("flag {flag} out of range 0–7") });
    }
